...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # initial all the beliefs to 1 and normalize will fix it to possibility
    ...
    def observe(self, observation, gameState):
        ...
        # handling special case if its captured
        if noisyDistance == None:
            allPossible[self.getJailPosition()]= 1.0
        else:
            # update the belief by using emissionModel[trueDistance] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            #print oldPos
            #print newPosDist
            #print self.beliefs[oldPos]
            for newPos, prob in newPosDist.items():
                # there might be alot oldPos heading to the newPos, so we need to add up every single possibility
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initially, we put all the particles evenly to all the legal positions
        uniformParticles = []
        num= self.numParticles
        while num > len(self.legalPositions):
            uniformParticles += (self.legalPositions)
            num -= len(self.legalPositions)

        uniformParticles += self.legalPositions[0:num]
        self.particlesList = uniformParticles
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        # handling special case if the ghost is captured
        if noisyDistance == None:
            self.particlesList = []
            i = 0
            ...
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # update the new beliefs
                allPossible[p] = emissionModel[trueDistance] * self.getBeliefDistribution()[p]
            # handling special case, if the total weight is 0, we will reinitialize
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                # use the particles to present the beliefs
                self.particlesList = []
                j = 0
    ...
    def elapseTime(self, gameState):
        ...
        # print len(self.particlesList)
        for oldPos in self.particlesList:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # because here newPosDist is exactly can be treat as a belief for the update of a oldpos.
            # so basically, we create new samples according to this distribution will get exact the result we want
            updatedParticles.append(util.sample(newPosDist))
        self.particlesList = updatedParticles
    ...
    def getBeliefDistribution(self):
        ...
        Counter object)
        """
        # actually we just transfer the distribution of particles to beliefs.
        # and we use normalize to get a possibility
        particlePossible = util.Counter()
        for particle in self.particlesList:
            particlePossible[particle] += 1

        particlePossible.normalize()
        return particlePossible
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # we use itertools.product to get Cartesian product repeat is actually product n times of the self.legalPositions
        ...
        uniformParticles = []
        num= self.numParticles
        while num > len(allGhostPositions):
            uniformParticles += (allGhostPositions)
            num -= len(allGhostPositions)

        uniformParticles += allGhostPositions[0:num]
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            possibility = 1
            for i in range(self.numGhosts):
                # handling special case if ghost is captured
                if noisyDistances[i] == None:
                    particle = self.getParticleWithGhostInJail(particle, i)
                else:
                    #update belief for each position for each ghost
                    emissionGhost = emissionModels[i]
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    possibility *= emissionGhost[trueDistance]
            allPossible[particle] += possibility

        # handling special case if totall weight is zero
        if allPossible.totalCount() == 0:
            self.initializeParticles()

        else:
            # giving a presentation according to the beliefs
            self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                # almost the same thing as question5, except we have to use the new given line to
                # calculate the distribution for the ghosts
